...
function jedenNumer(e) {
    ...
    <ul>
        <li v-for="i in info" >
            <span :id="i.geometry.coordinates[0]"> 
                Place: <b>{{ i.properties.place }}</b><br>Magnitude: <b>{{ i.properties.mag }} Richter scale</b></li>
            </span>
    </ul>
    ...
    for (var i = 0; i < m.length; i++) {
        ...
        var feature = new OpenLayers.Feature.Vector(
            ...
            { description: "marker number " + i, lon: lon, lat: lat, magnitude: magnitude },
...
function onFeatureSelect(feature) {
    currentSelected = feature.attributes.lon;
    document.getElementById(feature.attributes.lon).style.color = "#0033cc";
    document.getElementById(feature.attributes.lon).style.fontWeight = "900";
    document.getElementById(feature.attributes.lon).style.fontSize = "20px";

    var feature = new OpenLayers.Feature.Vector(
        new OpenLayers.Geometry.Point(feature.attributes.lon, feature.attributes.lat).transform(epsg4326, projectTo),
        { lon: feature.attributes.lon, lat: feature.attributes.lat, magnitude: feature.attributes.magnitude },
        { externalGraphic: 'img/marker2.png', graphicHeight: feature.attributes.magnitude, graphicWidth: feature.attributes.magnitude, graphicXOffset: -12, graphicYOffset: -25 }
    );
...
function onFeatureUnselect(feature) {
    if (currentSelected != undefined) {
        ...
        document.getElementById(currentSelected).style.color = "black";
        document.getElementById(currentSelected).style.fontWeight = "500";
        document.getElementById(feature.attributes.lon).style.fontSize = "15px";
        var feature = new OpenLayers.Feature.Vector(
            new OpenLayers.Geometry.Point(feature.attributes.lon, feature.attributes.lat).transform(epsg4326, projectTo),
            { lon: feature.attributes.lon, lat: feature.attributes.lat, magnitude: feature.attributes.magnitude },
            { externalGraphic: 'img/marker.png', graphicHeight: feature.attributes.magnitude, graphicWidth: feature.attributes.magnitude, graphicXOffset: -12, graphicYOffset: -25 }
        );
